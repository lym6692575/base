...
<!--            :disabled="!row.isEditing"-->
<!--          ></el-input>-->
          <znbm v-model="row['tgbm']" :disabled="!row.isEditing" :tooltip="true" :showAllOptions="false"></znbm>
        </template>
      </el-table-column>
...
<script>
import { znbm,fjlx } from "@/components/select";
export default {
  props: {
    value: {
      type: Array,
  ...
  computed: {
    ...
    tableData() {
      const tableData = [];
      const tdhgwjFj = this.tdhgwjFj;
      // console.log("tdhgwjFj", this.tdhgwjFj);
